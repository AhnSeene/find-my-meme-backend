...
public class FindPostController {
    ...
    @PostMapping
    public ResponseEntity<ApiResult<FindPostUploadResponse>> uploadFindPost(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "존재하지 않는 게시글", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
    @GetMapping("/{id}")
    public ResponseEntity<ApiResult<FindPostGetResponse>> getFindPost(
    ...
    @GetMapping
    public ResponseEntity<ApiResult<MyPage<FindPostSummaryResponse>>> getFindPosts(
    ...
    public ResponseEntity<ApiResult<MyPage<MyFindPostSummaryResponse>>> getMyFindPosts(
            @Parameter(description = "페이지 번호 (0부터 시작)", example = "0") @RequestParam(name = "page", defaultValue = "0") int page,
            @Parameter(description = "페이지 당 게시물 수", example = "5") @RequestParam(name = "size", defaultValue = "5") int size,
            @Parameter(description = "조회할 게시물 상태. **가능한 값: FIND, FOUND**", example = "FIND")
            @RequestParam(name = "status", required = false) FindStatus findStatus,
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "403", description = "게시글 수정 권한 없음", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 게시글", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
    @PutMapping("/{id}")
    public ResponseEntity<ApiResult<FindPostUpdateResponse>> updateFindPost(
            @PathVariable("id") Long findPostId,
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "403", description = "댓글 채택 권한 없음", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 게시글 또는 댓글", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "409", description = "이미 채택된 게시글", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
    @PostMapping("/{postId}/comments/{commentId}/select")
    public ResponseEntity<ApiResult<FindPostFoundResponse>> found(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "403", description = "게시글 삭제 권한 없음", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "404", description = "존재하지 않는 게시글", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
