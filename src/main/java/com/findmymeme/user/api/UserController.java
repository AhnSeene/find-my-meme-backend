...
public class UserController {
    ...
    @GetMapping("/me")
    public ResponseEntity<ApiResult<UserInfoResponse>> getMyInfo(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "잘못된 파일 요청 (e.g., 파일 비어있음)", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "401", description = "인증되지 않은 사용자", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
    @PostMapping("/profile-image")
    public ResponseEntity<ApiResult<UserProfileImageResponse>> updateProfileImage(
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "409", description = "이미 사용 중인 아이디", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
    @PostMapping("/check-username")
    public ResponseEntity<ApiResult<Void>> checkUsername(@Valid @RequestBody UsernameCheckRequest request) {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "입력값 유효성 검사 실패", content = @Content(schema = @Schema(implementation = ApiResult.class))),
            @ApiResponse(responseCode = "409", description = "이미 사용 중인 이메일", content = @Content(schema = @Schema(implementation = ApiResult.class)))
    })
    @PostMapping("/check-email")
    public ResponseEntity<ApiResult<Void>> checkEmail(@Valid @RequestBody EmailCheckRequest request) {
